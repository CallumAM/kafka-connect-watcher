...
def test_start_watcher_runs(monkeypatch):
    ...
    with patch(
        "kafka_connect_watcher.cli.Config", return_value=mock_config
    ) as config_patch, patch(
        "kafka_connect_watcher.cli.Watcher", return_value=mock_watcher
    ) as watcher_patch:
        start_watcher()
...
def test_start_watcher_passes_abspath(monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", test_args)
    with patch(
        "kafka_connect_watcher.cli.path.abspath", side_effect=lambda x: f"/abs/{x}"
    ) as abspath_patch, patch(
        "kafka_connect_watcher.cli.Config"
    ) as config_patch, patch(
        "kafka_connect_watcher.cli.Watcher"
    ) as watcher_patch:
